...
maven_and_environment_quirks:
  rules:
    ...
    - "If compiling on a Fedora-based OS where the system default is Java 25, be aware that Fedora's `mvn` wrapper script completely ignores `update-alternatives` and uses `/etc/java/maven.conf` if `JAVA_HOME` is not explicitly set in the environment. To ensure a successful build without modifying the global system, always advise setting `JAVA_HOME=/usr/lib/jvm/java-17-temurin-jdk` in a `~/.mavenrc` file so Maven correctly invokes Java 17 for the compiler and Lombok annotation processor."
    - "On Ubuntu/Debian systems, IDE internal terminals (like VS Code) may completely ignore `~/.mavenrc`. The `JAVA_HOME` must be explicitly exported in `~/.bashrc` (e.g., `export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64`) to prevent the environment from defaulting to the wrong Java version and crashing during compilation."
